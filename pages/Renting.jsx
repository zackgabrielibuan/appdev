...
function Renting() {
    ...
    useEffect(() => {
        ...
        const selectedCar = cars.find(car => car.id === parseInt(carId, 10));
        setCar(selectedCar);
    }, [carId, cars]);
    ...
    const handleRentCar = () => {
        alert(`You have successfully rented the ${car.brand}!`);
        // Add additional logic here for rental process, e.g., API call or state update
    };
    ...
    return (
        <div className="renting-page">
            <h1>Renting Page</h1>
            <h2>Selected Car: {car.brand}</h2>
            <div className="image-container">
                <img src={car.image} alt={car.brand} />
            </div>
            <p>Price: {new Intl.NumberFormat('en-PH', { style: 'currency', currency: 'PHP' }).format(car.price)} per day</p>
            <p>Type: {car.type}</p>

            {/* Rent Car Button */}
            <button onClick={handleRentCar} className="rent-button">
                Rent Car
            </button>
